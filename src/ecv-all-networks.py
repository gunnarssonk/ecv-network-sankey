#!/usr/bin/env python3
"""
Renders the full ECV - Networks Sankey published with the workbook "Observing
the GCOS Essential Climate Variables".

Every network and organisation is named, grouped under SATELLITE and the IN SITU
sub-categories. ecv-network-type.py draws the same links collapsed to the two
observing types, without naming anything.
...
- build_payload(flows): Groups and orders the networks, lists nodes and links.
...
- Run: python src/ecv-all-networks.py [--outdir DIR] [--no-open]
...
# ---------- FIGURE ----------
OUTPUT_NAME = "ecv-all-networks.html"
...
SUBTITLES = (        # format strings over the counts from subtitle_counts()
    ...
    "Cross-domain networks observe ECVs in more than one domain.",
)
...
def build_payload(flows):
    """Group and order the networks, list nodes and links."""
    is_insitu = flows["type_target"] == "In situ"
    group_of_network = flows[is_insitu].groupby("network")["ecv_domain"].apply(classify).to_dict()
    flows = flows.assign(group=flows["network"].map(group_of_network).where(is_insitu, SATELLITE_GROUP))
    group_order = [SATELLITE_GROUP, *DOMAINS.values(), CROSS]
...
<title>ECV to All Networks &amp; Organisations</title>
<style>
