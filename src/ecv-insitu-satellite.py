#!/usr/bin/env python3
"""
Renders the ECV - In Situ Networks and ECV - Satellite Organisations Sankeys
published with the workbook "Observing the GCOS Essential Climate Variables".

Two figures from one engine. The in situ figure names every in situ network,
grouped by the domain(s) it serves; the satellite figure names every satellite
data-record responsible organisation in one flat group.
...
- build_payload(flows, spec): Keeps one observing type and orders its nodes and links.
- subtitle_counts(payload): Counts what the figure draws, for the subtitle.
- render_html(payload, spec): Fills the HTML template with the data, the library and the figure's wording.
- build_figure(key, flows, outdir): Writes one figure. Returns its path.
...
- Run: python src/ecv-insitu-satellite.py [insitu|satellite|both] [--outdir DIR] [--no-open]
- Edit FIGURES to change what a figure says; edit the constants at the top of
...
# ---------- FIGURES: what differs between the two ----------
# Keyed by the command-line word; each figure is written to ecv-<key>-networks.html.
FIGURES = {
    "insitu": {
        "type_target": "In situ",       # value of "type (target)" to keep; also the column title
        "title": "GCOS Essential Climate Variables (ECVs) observed by in situ networks",
        "subtitles": (                  # format strings over the counts from subtitle_counts()
            "Of the {gcos_total} GCOS ECVs, {n_ecvs} are observed by {n_networks} in situ networks "
            "({n_links} links).",
            "Networks are grouped by the domains they serve; cross-domain networks observe ECVs in more than one domain.",
        ),
    },
    "satellite": {
        "type_target": "Satellite",
        "title": "GCOS Essential Climate Variables (ECVs) observed by satellite data-record organisations",
        "subtitles": (
            "Of the {gcos_total} GCOS ECVs, {n_ecvs} are observed by {n_networks} satellite data-record "
            "organisations ({n_links} links).",
        ),
    },
}
GCOS_ECV_TOTAL = 55  # ECVs in the GCOS list (GCOS-245, 2022); both subtitles count the mapped ones against it
...
def build_payload(flows, spec):
    """Keep one observing type, group and order its networks, list nodes and links."""
    flows = flows[flows["type_target"] == spec["type_target"]].copy()

    if spec["type_target"] == "In situ":  # in situ networks group by the domains they serve
        group_of_network = flows.groupby("network")["ecv_domain"].apply(classify).to_dict()
        flows["group"] = flows["network"].map(group_of_network)
        group_order = [*DOMAINS.values(), CROSS]
    else:                                 # satellite organisations sit in one flat group
        flows["group"] = SATELLITE_GROUP
        group_order = [SATELLITE_GROUP]
...
<title>ECV to Networks &amp; Organisations</title>
<style>
...
const ROW_R      = 12;   // network row pitch; below about 10 the names get tight
...
const netByKey  = Object.fromEntries(nets.map(d => [d.type + ":" + d.name, d]));
...
// ---- BLOCKS ----------------------------------------------------------------
// Each domain and network group is one continuous run of rows.
...
def subtitle_counts(payload):
    ...
    return {
        ...
        "n_networks": len({n["name"] for n in nets}),
...
def render_html(payload, spec):
    ...
    subtitle_lines = "\n".join(f"  {json.dumps(s.format(**counts))}," for s in spec["subtitles"])
    return (
        HTML_TEMPLATE.replace("__D3_SCRIPT__", d3_script())
        .replace("__DATA_JSON__", data_json)
        .replace("__TITLE__", spec["title"])
        .replace("__SUBTITLE_LINES__", subtitle_lines)
        .replace("__COLUMN_TITLE__", spec["type_target"].upper())
    )


def build_figure(key, flows, outdir):
    """Draw one figure and write it to outdir. Returns the path written."""
    spec = FIGURES[key]
    payload = build_payload(flows, spec)
